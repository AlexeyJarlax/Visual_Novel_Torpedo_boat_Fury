...
class GameRepositoryImpl(
    ...
    /**     * Обновление диалога с следующим фрагментом.     */
    ...
    /**     * Получение диалога по индексу. Зацикливание диалога для чтения книги, номера индексы - диалоги с книгами.     */
    ...
    /**     * Получение следующего куска книги. Фрагмент книги как кусок текста, который выдается вновь при нажатии кнопки пролистования.      */
    ...
    private fun playMusic() {
        ...
        mediaPlayer?.release() // Освобождаем предыдущий ресурс
        mediaPlayer = MediaPlayer.create(context, music.track)
        mediaPlayer?.isLooping = true // Looping the music track
        mediaPlayer?.start()

        // Устанавливаем слушатель завершения воспроизведения для проигрывателя музыки
        mediaPlayer?.setOnCompletionListener {
            // После завершения воспроизведения текущего трека, проигрываем следующий трек
            playNextMusic()
        }
    }
    ...
    private val dialogues: List<Pair<Int, Dialogue>> = listOf(     /** тут начинаются диалоги */
        ...
        25 to Dialogue(
            text = "Добравшись до Либавы мы с Новиковым направляемся в комментатуру флота доложить о прибытии. В коммендатуре все стоят на ушах - вот-вот флот должен выдвинуться в " +
        ...
        28 to Dialogue(
            text = "Кроме технических деталей похода, Рождественский пытался вдохновить офицеров перед операцией, которая вызывала беспокойство практически у всех, ведь мы идем на защиту " +
                    "крепости Порт-Артур, которая к моменту нашего пребытия вероятно уже падёт...",
            scene = scenes[2],
            options = listOf()
        ),
        ...
        36 to Dialogue(
            text = "$paramonov::: К Подозрительным личностям проявить особую бдительность, попытку проникнуть на боевое судно без документов грубо пресечь, " +
                    "а нарушителей арестовать - это приказ адмирала Рождественского... Ни знаю кто вы, сударъ, но вы совершили большую ошибку...",
            options = listOf()
        ),
        ...
        41 to Dialogue(
            text = "$paramonov::: Капитан 2-го ранга Алексеев - офицер торпедного вооружения. Капитан лейтенант Ибрагимов - навигационный офицер. Капитан-поручик Магометов и капитан-поручик " +
                    "Феодосов - офицеры торпедного вооружения. Капитан-поручик Шмель - офицер связи. Капитан-пороучик Резцов - офицер разведки.",
            options = listOf()
        ),

        42 to Dialogue(
            text = "$paramonov::: Подпоручик Гай - инженерные и ходовые системы. Карабельный врач Бухарин. К сожалению корабль недостаточно укомплектован механиками, инженерами и священником, " +
        ...
        45 to Dialogue(
            text = "2 октября 1904 года. Спал я тревожно и сон долго не шёл. Мне приснилось, как Грозный весь в пробоинах, со страшным креном и раненными матросами, лежащими на палубе на " +
                    "полном ходу уходит от попятам приследующего врага... Я встрехнул голову, прогоняя навождение и возвращаясь в реальность. Слышно, как гудят паровые машины, команды с " +
