...
class GameRepositoryImpl : GameRepository {
    ...
    private val dialogues: List<Pair<Int, Dialogue>> = listOf(
        ...
        Pair(
            22, Dialogue(
                text = "${Characters.novikov}:: В число команды входит немало новобранцев. Измученные и жалкие, они взирают на все вокруг с застывшим ужасом в глазах. Их пугает море, на которое они попали впервые, а ещё больше — неизвестное будущее. Даже среди кадровых моряков, окончивших различные специальные школы, не видно обычного веселья...",
                scene = scenes[2],
                options = listOf()
            )
        ),
        Pair(
            23, Dialogue(
                text = "${Characters.novikov}:: Только штрафные, в противоположность остальным, держатся более или менее бодро. Береговое начальство, чтобы отделаться от них, как от вредного элемента, придумало самый лёгкий способ: списывать их на суда, отправляющиеся на войну. Таким образом, к ужасу нашему, у нас набралось их до семи процентов...",
                scene = scenes[2],
                options = listOf()
            )
        ),
        ...
        Pair(
            50, Dialogue(
                text = "Танжер, Марокко. Тут эскадра должна разделиться. Часть судов, осадка которых позволяет пройти через Суэцкий канал, пойдёт под командой адмирала Фелькерзама этим путем. Рожественский с главными силами поведёт корабли кругом Африки. Оба отряда соединятся на Мадагаскаре",
                scene = scenes[3],
                options = listOf()
            )
        ),
...
//                        capital = true // Устанавливаем значение для этой опции
        ...
        //        «Что де́лать? Наболéвшие вопрóсы нáшего движéния» — В. И. Ленин
        /** ГЛАВА 5 Танжер, Марокко*/
        Pair(
            50, Dialogue(
                text = "Танжер, Марокко",
                scene = scenes[3],
        ...
        Pair(
            1000, Dialogue(
                text = "25 декабря 1904 года. Мадагаскар.",
                scene = scenes[4],
                options = listOf()
            )
        ),

        /** ГЛАВА 8 присоединился отряд Добротворского */
        Pair(
            1000, Dialogue(
